/*
 MVC패턴으로 개발하다보면 늘어나는 컨트롤러에대해 일일이 매핑과정을 진행해야한다. 
 이때 너무많은 매핑은 관리하기가 까다롭다. 따라서 유지보수성이 떨어진다. 
 현실과 유사하게, 어플리케이션에서도 대형 업무처리시 클라이언트의 요청을 곧바로 해당컨트롤러에게 처리하게하지않고,
 중간에 메인컨트롤러를 두고서 모든 요청을  이 메인 컨트롤러에서 처리하여 적절한 하위 컨트롤에게 전담시키는 방식을 이용한다. 
...
public class DispatcherServlet extends HttpServlet{
	...
	public void init(ServletConfig config) throws ServletException {
		//getRealPath는 jsp의 내장객체중 application에 대한 정보를 갖는 application 내장객체에서 지원한다. 
	...
	public void doRequest(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException
	{
		...
		System.out.println("지금들어온 요청은"+uri);
		//Controller controller;
		Controller controller=null;
		String className=null;
		
		//if문 대신에 프로퍼티스 객체를 이용하여 key값으로 메모리 올려질 컨트롤러의 이름을 value로 반환받자 
	...
	//서블릿의 생명주기 메서드 중 서블릿이 소멸할때 호출되는 메서드인 destory()에 , 스트림등의 자원은 닫는 처리를 하자 
	@Override
	public void destroy() {
		if(fis!=null) {
			try {
